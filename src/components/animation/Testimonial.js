    ...
    <div className="h-screen w-screen relative bg-gradient-to-r from-orange-400 to-rose-400 overflow-hidden flex justify-center flex-col items-center">
      ...
      <div className="relative flex justify-center pt-72">
        {data.map((obj, index) => {
          return (
            <span
              onClick={(w) => handleSetClick(index)}
              key={index}
              className={`h-4 w-4 m-1 rounded-full flex justify-center items-center cursor-pointer bg-white ${
                index === page
                  ? "transition delay-150 bg-blue-400 transform scale-125"
                  : ""
              }`}
            />
          );
